...
import { 
  Terminal, Send, Check, Copy, Sparkles, Code2, Cpu, Moon, Sun, 
  ArrowUpRight, X, Download, Github, Linkedin, Twitter, Zap, Layout, 
...
const Toast = ({ message, type, onClose }: { message: string, type: 'success' | 'error' | 'info', onClose: () => void }) => {
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-6 left-12 transform -translate-x-1/2 z-[100] px-4 py-3 rounded-full shadow-2xl flex items-center gap-3 border 
        bg-red-500/10 border-red-500/20 text-red-500 
        data-[type=success]:bg-green-500/10 data-[type=success]:border-green-500/20 data-[type=success]:text-green-500
        data-[type=info]:bg-white dark:data-[type=info]:bg-[#18181b] data-[type=info]:border-gray-200 dark:data-[type=info]:border-white/10 
        data-[type=info]:text-gray-900 dark:data-[type=info]:text-gray-100 backdrop-blur-md"
      data-type={type}
    >
...
export default function Home() {
  ...
  const [selectedTone, setSelectedTone] = useState<'Professional' | 'Casual' | 'Storyteller' | 'ELI5'>('Professional');
  ...
  const tones = ['Professional', 'Casual', 'Storyteller', 'ELI5'] as const;
  const suggestions = [
  ...
  useEffect(() => {
    const handleTyping = () => {
      ...
      if (!isDeleting && heroText === fullText) {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `${topic.slice(0, 20).replace(/\s+/g, '-')}.blog.md`;
  ...
  const scrollbarClass = `overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-neutral-700 
    scrollbar-track-transparent scrollbar-thumb-rounded-full 
    hover:scrollbar-thumb-gray-400 dark:hover:scrollbar-thumb-neutral-600`;

  return (
    <div className="transition-colors duration-500 bg-[#fafafa] dark:bg-[#050505] text-gray-900 dark:text-gray-200 font-sans selection:bg-purple-500/30 selection:text-purple-300 flex flex-col">
      <AnimatePresence>
        {toast && (
          <Toast
      ...
      <motion.header
        ...
        className="fixed top-0 w-full z-50 bg-white/70 dark:bg-[#050505]/70 backdrop-blur-xl border-b border-gray-200 dark:border-white/5 h-16"
        style={isFullscreen ? { display: 'none' } : { display: 'block' }}
      >
        <div className="w-full max-w-7xl mx-auto px-6 h-full flex justify-between items-center">
          ...
          >
            ...
            <div>
              <span className="text-xl font-bold tracking-tight text-gray-900 dark:text-white">Floc</span>
              <span className="text-purple-600 dark:text-purple-500">Care</span>
              <span className="text-purple-600 dark:text-purple-500">.ai</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            <nav className="hidden md:flex gap-8 text-sm font-medium text-gray-500 dark:text-gray-400">
              ...
              <button
                onClick={() => {
                  setIsCanvasOpen(false);
                  setTimeout(() => scrollToSection('tech-stack'), 100);
                }}
                className="hover:text-purple-600 dark:hover:text-white transition-colors"
              >
                Stack
              ...
              <button
                onClick={() => {
                  if (!isCanvasOpen) scrollToSection('generator');
                }}
                className="text-purple-600 dark:text-purple-400 font-semibold bg-purple-500/10 px-3 py-1 rounded-full"
              >
                Generator
              </button>
            </nav>
      ...
      {/* MAIN CONTAINER */}
      <div className="flex-1 flex relative min-h-0" style={{
        paddingTop: isCanvasOpen ? (isFullscreen ? '0' : '4rem') : '4rem'
      }}>
        {/* LEFT PANEL - Input */}
        <motion.div
          layout
          ...
          className={`flex flex-col px-6 transition-all ${
            isCanvasOpen
              ? isFullscreen
                ? 'hidden'
                : 'w-full md:w-[35%] border-r border-gray-200 dark:border-white/5 pt-6 justify-start h-full'
              : 'flex w-full max-w-5xl mx-auto justify-start'
          } ${scrollbarClass}`}
          style={mobileTab === 'output' ? { display: 'none' } : { display: 'flex' }}
        >
          {/* HERO */}
          <AnimatePresence>
            {!isCanvasOpen && (
          ...
          <motion.div
            layout
            id="generator"
            className="relative w-full z-10 mb-20"
          >
            <div className="absolute -inset-1 bg-gradient-to-r from-purple-600 to-blue-600 rounded-2xl opacity-20 blur-xl group-hover:opacity-40 transition duration-1000" />
            <div className={`relative bg-white dark:bg-[#121212] rounded-2xl border border-gray-200 dark:border-white/10 shadow-xl overflow-hidden group focus-within:ring-2 ring-purple-500/50 transition-all ${
              ...
              <textarea
                ...
                className="w-full bg-transparent text-gray-900 dark:text-gray-100 p-6 outline-none font-medium resize-none placeholder:text-gray-400 dark:placeholder:text-gray-600
                  h-32 text-base md:h-[200px] md:text-lg"
              />
              {!isCanvasOpen && !topic && (
              ...
              <div className="flex flex-col gap-4 p-4 border-t border-gray-200 dark:border-white/5 bg-gray-50 dark:bg-[#18181b]">
                ...
                <div className="flex justify-between items-center">
                  ...
                  {loading || isTyping ? (
                    <button
                      onClick={handleStop}
                      className="px-6 py-2.5 rounded-lg font-bold text-sm transition-all flex items-center gap-2 bg-red-100 text-red-500 dark:bg-red-500/10 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-500/20"
                    >
                    ...
                    >
                      <span>Generate</span>
                      <span className="hidden lg:inline text-[10px] text-gray-400 bg-gray-800 px-1.5 py-0.5 rounded ml-1 opacity-70">
                        ⌘+⏎
                      </span>
            ...
            {!isCanvasOpen && (
              ...
              >
                ...
                <p className="text-gray-500 max-w-xl mx-auto">
                  Built to demonstrate the power of Next.js 14 combined with advanced AI streaming.
                </p>
                  ...
                  {[
                    {
                      ...
                      icon: <svg viewBox="0 0 180 180" className="w-8 h-8 fill-black dark:fill-white"><path d="M149.508 157.52L69.142 54H54V125.97H66.1136V69.3836L139.999 164.845C143.333 162.614 146.509 160.165 149.508 157.52Z"/><rect height="72" width="12" x="115" y="54"/></svg>
                    },
                    {
                      name: 'React',
                      color: 'hover:shadow-blue-500/20',
                      icon: <svg viewBox="-10.5 -9.45 21 18.9" className="w-8 h-8 fill-[#61dafb]"><circle cx="0" cy="0" r="2"/><circle stroke="#61dafb" strokeWidth="1" fill="none" r="2" cx="0" cy="0"/><ellipse rx="10" ry="4.5"/><ellipse rx="10" ry="4.5" transform="rotate(60)"/><ellipse rx="10" ry="4.5" transform="rotate(120)"/></svg>
                    },
                    {
                      name: 'Tailwind',
                      color: 'hover:shadow-cyan-500/20',
                      icon: <svg viewBox="0 0 24 24" className="w-8 h-8 fill-[#38bdf8]"><path d="M12.001 4.8c-3.2 0-5.2 1.6-6 4.8 1.2-1.6 2.6-2.2 4.2-1.8.913.228 1.565.89 2.288 1.624C13.666 10.618 15.027 12 18.001 12c3.2 0 5.2-1.6 6-4.8-1.2 1.6-2.6 2.2-4.2 1.8-.913-.228-1.565-.89-2.288-1.624C16.337 6.182 14.976 4.8 12.001 4.8zm-6 7.2c-3.2 0-5.2 1.6-6 4.8 1.2-1.6 2.6-2.2 4.2-1.8.913.228 1.565.89 2.288 1.624 1.177 1.194 2.538 2.576 5.512 2.576 3.2 0 5.2-1.6 6-4.8-1.2 1.6-2.6 2.2-4.2 1.8-.913-.228-1.565-.89-2.288-1.624C10.337 13.382 8.976 12 6.001 12z"/></svg>
                    },
                    {
                      name: 'Gemini AI',
                      color: 'hover:shadow-purple-500/20',
                      icon: <svg viewBox="0 0 24 24" className="w-8 h-8 fill-purple-600 dark:fill-purple-400"><path d="M22 11.08c-6.24 0-10.92-4.96-10.92-11.08 0 6.12-4.68 11.08-10.92 11.08 6.24 0 10.92 4.96 10.92 11.08 0-6.12 4.68-11.08 10.92-11.08z"/></svg>
                    }
                  ].map((tech, i) => (
                    <motion.div
                      key={i}
                      variants={scaleUp}
                      className={`p-6 rounded-2xl bg-white dark:bg-[#121212] border border-gray-200 dark:border-white/5 hover:border-black dark:hover:border-white/20 transition-all flex flex-col items-center gap-4 hover:-translate-y-1 shadow-sm group ${tech.color} hover:shadow-xl`}
                    >
              ...
              >
                <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                  ...
                  <div className="flex gap-4">
                    <a href="https://github.com/Nishmith12" target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-all">
                      <Github className="w-5 h-5" />
                    </a>
                    <a href="https://www.linkedin.com/in/nishmith-kumar-54162026a" target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 hover:bg-[#0077b5] hover:text-white transition-all">
                      <Linkedin className="w-5 h-5" />
                    </a>
                    <a href="https://twitter.com" target="_blank" rel="noopener noreferrer" className="p-2 rounded-full bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-all">
        ...
        <AnimatePresence>
          {isCanvasOpen && (
            <motion.div
              ...
              transition={{ type: "spring", bounce: 0, duration: 0.6 }}
              className="flex-col flex-1 bg-white dark:bg-[#0a0a0a] border-l border-gray-200 dark:border-white/5 h-full relative shadow-2xl overflow-hidden"
              style={mobileTab === 'input' ? { display: 'none' } : { display: 'flex' }}
            >
              {/* CANVAS HEADER */}
              <div className="h-14 border-b border-gray-200 dark:border-white/5 flex items-center justify-between px-6 bg-gray-50/50 dark:bg-[#0a0a0a]">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="p-1.5 hover:bg-gray-200 dark:hover:bg-white/10 rounded-md text-gray-500 hover:text-red-500 transition-colors"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-10 bg-white dark:bg-[#0a0a0a] scrollbarClass">
                <div className="relative max-w-3xl mx-auto">
                  ...
                  <div className="absolute -inset-4 bg-gradient-to-r from-purple-600/30 to-blue-600/30 rounded-3xl opacity-30 blur-3xl pointer-events-none" />
                  ...
                  <div className="relative prose prose-lg max-w-none dark:prose-invert">
                    {loading && !displayedContent ? (
                      <div className="space-y-6 animate-pulse mt-10">
                        ...
                        <div className="h-4 bg-gray-200 dark:bg-white/5 rounded-full w-full" />
                        <div className="h-4 bg-gray-200 dark:bg-white/5 rounded-full w-5/6" />
                        <div className="h-40 bg-gray-200 dark:bg-white/5 rounded-xl w-full" />
                      </div>
                    ) : (
                      <motion.div
                      ...
                      >
                        <ReactMarkdown
                          components={{
                            ...
                            code: ({ node, inline, className, children, ...props }: any) => {
                              return !inline ? (
                                <div className="relative group my-6">
                                  <div className="absolute -inset-2 bg-gradient-to-r from-purple-500/20 to-blue-500/20 rounded-xl blur-lg opacity-0 group-hover:opacity-100 transition duration-500" />
                            ...
                            a: ({ node, ...props }) => (
                              <a className="text-purple-600 dark:text-purple-400 hover:underline decoration-purple-500/30 underline-offset-2" {...props} />
                            ),
                          }}
                        >
